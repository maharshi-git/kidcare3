...
const TopNavbar = () => {
  ...
  return (
    <Navbar bg="light" variant="light" style={{"padding": "0rem"}} className="justify-content-between">
      ...
      <Nav>
        {(location.pathname !== '/Login') && <Button variant="primary" onClick={() => {navigate("./Login", {state: {headerText: "Hi! please login to start from where you left"}})}} style={{"width": "6rem"}} className='btn btn-success smallMargin appButtonPurple'>Sign In</Button>}
      </Nav>
    </Navbar>
  );
};
